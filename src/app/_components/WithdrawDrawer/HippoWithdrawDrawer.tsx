...
import { HIPPO_ADDR } from '@/app/contracts/address';
...
const WithdrawDrawer: React.FC<WithdrawDrawerProps> = ({ isOpen, onClose }) => {
	...
	const { claimId, fetchClaimData } = useFetchSignData();
	...
	const handleConfirm = useCallback(async () => {
		...
		try {
			...
			const claimData = await fetchClaimData(HIPPO_ADDR);
	...
	}, [loadingClaim, fetchClaimData, pollWithdrawStatus]);
	...
	return (
		<>
			{visible && (
				...
				>
					<div
						className={`flex h-screen w-full transform flex-col items-center justify-start bg-white bg-bg-blue bg-cover px-3 transition-transform duration-300 sm:max-w-[520px] ${
					...
					>
						...
						<div className="relative mb-5 mt-8 flex h-[120px] w-full items-center justify-center bg-blue-balance-panel bg-[length:100%_100%] bg-no-repeat">
							<Link
								href={'/withdraw/hippo'}
								onClick={clickRouter}
