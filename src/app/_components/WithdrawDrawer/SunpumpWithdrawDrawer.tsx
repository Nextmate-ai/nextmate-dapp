...
import { TRX_ZERO_ADDR } from '@/app/contracts/address';
...
const SunpumpWithdrawDrawer: React.FC<WithdrawDrawerProps> = ({
	isOpen,
	onClose,
}) => {
	...
	const handleWithdraw = async () => {
		...
		const claimData = await fetchClaimData(TRX_ZERO_ADDR);
		if (!claimData) {
	...
	return (
		<>
			{visible && (
				...
				>
					<div
						className={`flex h-screen w-full transform flex-col items-center justify-start bg-white bg-bg-orange bg-cover px-3 transition-transform duration-300 sm:max-w-[520px] ${
					...
					>
						...
						<div className="relative mb-5 mt-8 flex h-[120px] w-full items-center justify-center bg-red-balance-panel bg-[length:100%_100%] bg-no-repeat">
							...
							<div className="flex items-center justify-center gap-4">
								<img className="h-12 w-12" src="/img/TRX.png" alt="trx" />
...
export default SunpumpWithdrawDrawer;
