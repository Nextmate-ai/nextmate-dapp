...
const RoulettePage: React.FC = () => {
	...
	const { roulette, poolId } = useLotteryPrizes('VoteSundog');
			...
			const lucky = new LuckyWheel(
				...
				},
				{
					...
					prizes: roulette.map((prize, index) => ({
						background: index % 2 === 0 ? '#FFE716' : '#E0423B',
						imgs: [
							{
								src: prize.image,
								width: '45%',
	...
	return (
		<div className="no-scrollbar flex h-screen w-full flex-col overflow-x-hidden overflow-y-scroll bg-bg-blue bg-cover">
			<div className="flex flex-grow flex-col items-center justify-start">
				...
				>
					<div className="flex h-7 w-full items-center justify-center gap-1">
						<img src="/img/logo/sundog.png" alt="sundog" className="w-4" />
				...
				{/* roulette */}
				<div
					className={`relative -mt-[250px] h-[580px] w-[580px] rotate-180 bg-[length:100%_100%] bg-no-repeat ${isRendered ? 'bg-red-roulette' : 'animate-pulse rounded-full bg-gray-300'}`}
				>
					<div id="lucky-lottery" className="mx-auto w-full"></div>
					{isRendered && (
						<div className="absolute -top-7 left-1/2 z-20 -translate-x-1/2">
							<img
								src="/img/roulette/red-point.png"
								alt="point"
				...
				<div className="w-full px-2 text-center font-jamjuree text-sm font-normal text-white">
					The campaign will run for 14 days or until all $Sundog tokens are
				...
				<div className="relative mt-6 flex w-full justify-center">
					...
					>
						<img
							src={
								isActive ? '/img/red-btn.png' : '/img/bg-spin-btn-inactive.png'
							}
							alt="spin button"
						...
						<div className="absolute left-1/2 top-1/2 w-44 -translate-x-1/2 -translate-y-1/2">
							<div className="flex items-center justify-center">
								...
								<div className="text-center font-chakra text-base font-semibold tracking-wide text-white">
									SPIN x {user?.voteSundogDrawCount || 0}
								</div>
							</div>
						</div>
						{isLoading && (
