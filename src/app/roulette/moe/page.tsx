...
import WithdrawDrawer from '@/app/_components/WithdrawDrawer/MoeWithdrawDrawer';
...
const RoulettePage: React.FC = () => {
	...
	const [images, setImages] = useState([
		...
		'/img/gray-balance-panel.png',
		'/img/gray-history-border.png',
	]);
	...
	const { roulette, poolId } = useLotteryPrizes('Moe');
			...
			const lucky = new LuckyWheel(
				...
				},
				{
					...
					prizes: roulette.map((prize, index) => ({
						background: index % 2 === 0 ? '#FFE716' : '#414B6F',
						imgs: [
							{
								src: prize.image,
								width: '45%',
	...
	return (
		<div className="no-scrollbar flex h-screen w-full flex-col overflow-x-hidden overflow-y-scroll bg-bg-blue bg-cover">
			<div className="flex flex-grow flex-col items-center justify-start">
				...
				>
					<div className="flex h-7 w-full items-center justify-center gap-1">
						<img src="/img/logo/moe.png" alt="moe" className="w-4" />
				...
				{/* roulette */}
				<div
					className={`relative -mt-[250px] h-[580px] w-[580px] rotate-180 bg-[length:100%_100%] bg-no-repeat ${isRendered ? 'bg-gray-roulette' : 'animate-pulse rounded-full bg-gray-300'}`}
				>
					<div id="lucky-lottery" className="mx-auto w-full"></div>
					{isRendered && (
						<div className="absolute -top-7 left-1/2 z-20 -translate-x-1/2">
							<img
								src="/img/roulette/gray-point.png"
								alt="point"
				...
				{/* roulette text */}
				<div className="mt-3 w-full text-center font-jamjuree text-3xl font-bold italic leading-9 text-[#ffec47] text-shadow-yellow">
					Spin and win
				</div>
				<div className="mb-3 w-full text-center text-3xl font-bold italic leading-9 text-[#ffec47] text-shadow-yellow">
					217,000 $MOE rewards!
				</div>

				<div className="w-full px-3 text-center font-jamjuree text-sm font-normal text-white">
					The campaign will run for 14 days or until all $Moe tokens are
				...
				<div className="relative mt-6 flex w-full justify-center">
					...
					>
						<img
							src={
								isActive ? '/img/gray-btn.png' : '/img/bg-spin-btn-inactive.png'
							}
							alt="spin button"
