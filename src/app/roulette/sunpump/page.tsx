...
import SunpumpWithdrawDrawer from '@/app/_components/WithdrawDrawer/SunpumpWithdrawDrawer';
...
const RoulettePage: React.FC = () => {
	...
	const [images, setImages] = useState([
		...
		'/img/icon/light.svg',
		'/img/TRX.png',
		'/img/red-balance-panel.png',
		'/img/orange-history-border.png',
	]);
	...
	const { roulette, poolId } = useLotteryPrizes('Sunpump');
			...
			const lucky = new LuckyWheel(
				...
				},
				{
					...
					prizes: roulette.map((prize, index) => ({
						background: index % 2 === 0 ? '#FFE716' : '#FB9147',
						imgs: [
							{
								src: prize.image,
								width: '45%',
	...
	return (
		<div className="no-scrollbar flex h-screen w-full flex-col overflow-x-hidden overflow-y-scroll bg-bg-blue bg-cover">
			<div className="flex flex-grow flex-col items-center justify-start">
				...
				>
					<div className="flex h-7 w-full items-center justify-center gap-1">
						<img src="/img/TRX.png" alt="sunpump" className="w-4" />
				...
				{/* roulette */}
				<div
					className={`relative -mt-[250px] h-[580px] w-[580px] rotate-180 bg-[length:100%_100%] bg-no-repeat ${isRendered ? 'bg-orange-roulette' : 'animate-pulse rounded-full bg-gray-300'}`}
				>
					<div id="lucky-lottery" className="mx-auto w-full"></div>
					{isRendered && (
						<div className="absolute -top-7 left-1/2 z-20 -translate-x-1/2">
							<img
								src="/img/roulette/orange-point.png"
								alt="point"
				...
				{/* roulette text */}
				<div className="mt-3 w-full text-center font-jamjuree text-4xl font-bold italic leading-9 text-[#ffec47] text-shadow-yellow">
					Spin and win
				</div>
				<div className="mb-3 w-full text-center text-4xl font-bold italic leading-9 text-[#ffec47] text-shadow-yellow">
					10,000 TRX rewards!
				</div>

				<div className="w-full px-2 text-center font-jamjuree text-sm font-normal text-white">
					The campaign will run until December 31, 2024, or until all $TRX
					tokens are distributed, whichever comes first.
				</div>
